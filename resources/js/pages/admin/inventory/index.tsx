...
export default function InventoryIndex({ parts, summary, scooters, installed_count, can_manage_finance }: Props) {
    ...
    function deletePart(partId: number, partName: string) {
        const confirmed = window.confirm(`Weet je zeker dat je "${partName}" wilt verwijderen? Deze actie kan niet ongedaan worden gemaakt.`);

        if (!confirmed) {
            return;
        }

        router.delete(`/admin/voorraad/onderdelen/${partId}`, {
            preserveScroll: true,
            onSuccess: () => {
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            {can_manage_finance && (
                                                <>
                                                    ...
                                                    <button
                                                        type="button"
                                                        onClick={() => deletePart(selectedPart.id, selectedPart.name)}
